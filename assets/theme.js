...
(function($){
      ...
      open: function(evt, returnFocusEl) {
        ...
        if (this.config.id == "CartDrawer") {
          ...
          ga('send', {
            ...
            eventAction: 'Viewed Cart - Drawer',
          });
        }
        this.bindEvents();
      },
    ...
    QtySelector.prototype = $.extend({}, QtySelector.prototype, {
      initEventListeners: function() {
        this.$plus.on('click', function() {
          ...
          if ( $(this.$input).attr('product-id') == '39477865349216' ) {
            var bundle_id = $(this.$input).attr('bundle-id')
            changeAjaxQuantity(bundle_id, qty, true)
          } else if(qty != parseInt($(this.$input).attr('max'))) {
            this.addQty(qty);
          }
        }.bind(this));
        ...
        this.$minus.on('click', function() {
          ...
          if ( $(this.$input).attr('product-id') == '39477865349216' ) {
            var bundle_id = $(this.$input).attr('bundle-id')
            changeAjaxQuantity(bundle_id, qty, false)
          } else {
      ...
      buildCart: function(cart, openDrawer, returnFocusEl) {
        ...
        if (cart.item_count === 0) {
          ...
          $.each(cart.items, function(index, product) {
            ...
            var discount = 0;
            quantity = 0
            if(product.id == 39477865349216) {
              var bundleitems = cart.items.filter(item => item.properties?.bundle_id == product.properties?.bundle_id && item.id != 39477865349216)
              ...
              bundleitems.forEach(item => subnames.push(item.title))
            }
            ...
            item = {
              ...
              price: product.id == 39477865349216 ? theme.Currency.formatMoney(price, theme.settings.moneyFormat) : theme.Currency.formatMoney(product.price, theme.settings.moneyFormat),
          ...
          data = {
            ...
            tierRewards: `
                   <div class="tier-wrapper-drawer to-toggle main-toggle seed">
                      <span class="icon">
                        <img width="42" height="42" src="https://cdn.shopify.com/s/files/1/1777/6857/files/seed-icon.png?v=1645649280" alt="">
                      </span>
                      <div class="tier-name-drawer"><b>Seed</b> | Entry Level</div>
                      <div class="tier-earned-drawer"><span id="pseed"></span> pts</div>
                      <span class="chevron">
                   ...
                   <div class="tier-wrapper-drawer to-toggle bloom">
                      <span class="icon">
                        <img width="42" height="42" src="https://cdn.shopify.com/s/files/1/1777/6857/files/Bloom-Icon.png?v=1645649280" alt="">
                      </span>
                      <div class="tier-name-drawer"><b>Bloom</b> | Spend $1,000</div>
                      <div class="tier-earned-drawer"><span id="pbloom"></span> pts</div>
                      <span class="chevron">
                          <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14" fill="currentColor" class="bi bi-chevron-up" viewBox="0 0 16 14">
                             <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                          </svg>
                   ...
                   <div class="tier-wrapper-drawer to-toggle boll">
                      <span class="icon" style="position: relative;">
                        <img width="36" height="36" src="https://cdn.shopify.com/s/files/1/1777/6857/files/BOLL-ICON.png?v=1645649280" alt="">
                      </span>
                      <div class="tier-name-drawer"><b>Boll</b> | Spend $2,000</div>
                      <div class="tier-earned-drawer"><span id="pboll"></span> pts</span></div>
                      <span class="chevron">
                          <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14" fill="currentColor" class="bi bi-chevron-up" viewBox="0 0 16 14">
                             <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                          </svg>
        ...
        document.dispatchEvent(new CustomEvent('freeShippingBar:updated', {
          detail: {
            cart: cart
          }
        }));
  ...
  theme.Price = (function() {
    
    function bundlePriceUpdate($collection, __count, __price) {
      // $collection.find('.product-section').each(function() {
        var $comparePrice = $collection.find('.comparePrice');
        var $price = $collection.find('.price');
        var $discount_percent = $collection.find('.discount_percent');
        var addMoreDiv = $('.payment-buttons .bundle-content__product_quantity')
        var addMoreDiv = $('.payment-buttons .bundle-content__product_quantity')
        var addMoretext = ''

        if(__count > 0) {
          $collection.find('.bundle-content__bundle_text').removeClass('hide')
        } else {
          $collection.find('.bundle-content__bundle_text').addClass('hide')
        }
        
        if (__count == 1 ){
          addMoretext = "ADD 1 " + "MORE TO GET " + 5 +"% " + "OFF"
          addMoreDiv.text(addMoretext)
        }else if (__count == 2) {
          addMoretext = "ADD 1 " + "MORE TO GET " + 10 +"% " + "OFF"
          addMoreDiv.text(addMoretext)
        } else if(__count === 3) {
          addMoretext = "ADD 1 " + "MORE TO GET " + 15 +"% " + "OFF"
          addMoreDiv.text(addMoretext)
        } else {
          addMoreDiv.text(addMoretext)
        }
    ...
    function getBundlePrice(__count, __price) {
    ...
    return {
      bundlePriceUpdate: bundlePriceUpdate,
  ...
  theme.Product = (function() {
    ...
    function Product(containertop) {
      var containerAll = containertop.parentElement.querySelectorAll('.product-section');
      var bundle_img_urls = [];
      var bundle_cart_array = [];
      var bundle_cart_objs = {};
      containerAll.forEach((container, index) => {
        var $container = this.$container = $(container);
        var sectionId = this.sectionId = $container.attr('data-section-id');
        ...
        this.selectors = {
          ...
          addToBundle: '.add-item-to-bundle',
          addToBundleText: '.add-item-to-bundle-text',
          addBundleToCart: '.add-bundle-to-cart',
          quickAddToBundle: '.quick-add-to-bundle'
        };

        const bundleWrapID = 39477865349216;
        $(this.selectors.addToBundle).on('click', function(e) {
          ...
          var variant_title = $option.text()
          var img_url = $option.attr('data-image');
          var price = $option.attr('data-price');
          var variantOption1 = $option.attr('variant-option-1')
          var variantOption2 = $option.attr('variant-option-2')
          var variantOption3 = $option.attr('variant-option-3')
          var options = $option.attr('options')
          var product_select = $option.attr('product-select')
          var cart_obj = {};
          var key = variant_id;
          ...
          // bundle_img_urls.map((item) => {
            ...
            // $target.append('<img src="' + img_url + '">');
            // $target.addClass('active');
          // })

          // adding quantity of product added to tab

          var selection = $(`.bundle-tab__item[data-collection-id=${product_select}] .bundle-tab__item-count`)
          var item_count = 0, total_price = 0;
          ...
          $(this).closest('#CollectionSection').find('form').each(function(idx) {
            for(let i = 0; i < parseInt(qty); i++) {
              ...
              var $target = $(this).find(`.bundle-content__bundle_imageitem[data-index=${next_to_fill}]`);
              var selectionTab = $(`.bundle-tab__item[data-collection-id=${$target.attr('product-select')}] .bundle-tab__item-count`)
              if (idx == 0) {
                var count = parseInt(selectionTab.attr('data-count'))
                if (count - 1 > 0) {
                  selectionTab.attr('data-count', count - 1)
                  selectionTab.text(count - 1 )
                  selectionTab.removeClass('hide')
                } else if( count - 1 == 0 ) {
                  selectionTab.attr('data-count', count)
                  selectionTab.text('')
                  selectionTab.addClass('hide')
               }
              }
              $target.attr('product-select')
              $target.removeClass('active');
              $target.find('img').remove();
              $target.attr('data-variant-id', variant_id);
              $target.attr('data-variant-title', title);
              $target.attr('data-qty', 1);
              $target.attr('data-price', price);
              $target.attr('option-1', variantOption1);
              $target.attr('option-2', variantOption2);
              $target.attr('option-3', variantOption3);
              $target.attr('options', options);
              $target.append('<img src="' + img_url + '">');
              $target.addClass('active');
              $target.attr('variant-title', variant_title)
              $target.attr('product-select', product_select)
              if ((parseInt(next_to_fill) == 4)) {
                $(this).find(".bundle-content__bundle_imageitem").each(function() {
            ...
            if(idx == 0){
              $(this).find(".bundle-content__bundle_imageitem.active").each(function() {
                item_count ++;
                total_price += parseInt($(this).attr('data-price')) * parseInt($(this).attr('data-qty'));
              })
             var tabcount = findTabCount($(this).find(".bundle-content__bundle_imageitem.active"), product_select)
             if (tabcount > 0) {
              selection.attr('data-count', tabcount)
              selection.text(tabcount)
              selection.removeClass('hide')
             } else {
              selection.attr('data-count', 0)
              selection.text('')
              selection.addClass('hide')
             }
            }
            if($(this).find(".bundle-content__bundle_imageitem.active").length >= 1) {
              $(this).find('.bundle-content__bundle_wrapper .payment-buttons').removeClass('hide')
              $(this).find('.empty-bundle-text-container').addClass('hide')
            }
            if($(this).find(".bundle-content__bundle_imageitem.active").length == 1) {
            ...
            } else if ($(this).find(".bundle-content__bundle_imageitem.active").length > 1) {
              $(this).find('.bundle-content__bundle_wrapper .payment-buttons .add-bundle-to-cart').removeClass('disable')
              $(this).find('.bundle-content__bundle_wrapper .payment-buttons .add-bundle-to-cart').prop('disabled', false)
            }

            // if ($(this).find(".bundle-content__bundle_imageitem.active").length >= 4) {
            //   $(this).find(".bundle-content__bundle_imageitem.active").each(function() {
            //     img = $(this).find("img")[0]
            //     var data = {
            ...
            //       title: $(this).attr('data-variant-title'),
            //       options: $(this).attr('options').split(','),
            //       optionvalues: [$(this).attr('option-1'), $(this).attr('option-2'), $(this).attr('option-3')],
            //       dataIndex: $(this).attr('data-index'),
            //       dataQuantity: $(this).attr('data-qty'),
            //       product_select: $(this).attr('product-select'),
            //     }
            //     bundle_summary.push(data)
            //   })
            //   var content_htm = '';
            //   bundle_summary.forEach(bundle_item_data => {
            //     var item_htm = `<div class="atc_bundle_item d-flex" data-id=${bundle_item_data.id} data-price=${bundle_item_data.price} data-index=${bundle_item_data.dataIndex}>`;
            //     item_htm += '<div class="image_wrapper">';
            //     item_htm += '<img src="' + bundle_item_data.img_src + '">';
            //     item_htm += '</div>';
            //     item_htm += '<div class="item_info">'
            //     item_htm += '<div class="item_title">' + bundle_item_data.dataQuantity + ' x '  + bundle_item_data.title + '</div>';
            //     if(bundle_item_data.options.length > 0) {
            //       item_htm += '<div class="item_options">';
            //     }
            //     for(var j = 0; j < bundle_item_data.options.length; j ++) {
            //       item_htm += '<span>' + bundle_item_data.options[j] + ': ' + bundle_item_data.optionvalues[j] + '</span>';
            //     }
            //     if(bundle_item_data.options.length > 0) {
            //       item_htm += '</div>';
            //     }
            //     item_htm += '</div>';
            //     item_htm += `<div class="item_remove-product"><button data-index=${bundle_item_data.dataIndex} product-select=${bundle_item_data.product_select} data-qty=${bundle_item_data.dataQuantity}><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 371.23 371.23" style="enable-background:new 0 0 371.23 371.23;" xml:space="preserve"><polygon points="371.23,21.213 350.018,0 185.615,164.402 21.213,0 0,21.213 164.402,185.615 0,350.018 21.213,371.23   185.615,206.828 350.018,371.23 371.23,350.018 206.828,185.615 "/></svg></button><div class="edit-popup-product">Edit</div></div>`;
            //     item_htm += '</div>';
            //     content_htm += item_htm;
            //   })
            //   $(this).find('.popup-bundle .popup-content').html(content_htm);
            //   popup_open($(this).find('.popup-bundle'))
            //   popupPriceUpdate($(this).closest('.popup-bundle'), item_count, total_price)
            // }
          })

          $('.collection_bundlelist').each(function() {
            theme.Price.bundlePriceUpdate($(this), item_count, total_price);
          })
        })

        $(this.selectors.quickAddToBundle).on('click', function(e) {
          e.stopImmediatePropagation();
          var __bundleID = new Date().valueOf();
          var bundle_items = [];
          var BundleWrapproperties = {bundle_id: __bundleID}
          $(this).closest('.popup-bundle').find('.atc_bundle_item').each(function() {
            var bundle_item = {};
            bundle_item['id'] = $(this).attr('data-id');
            bundle_item['quantity'] = 1;
            bundle_item['properties'] = {bundle_id: __bundleID, type: 'bundle'};
            BundleWrapproperties[$(this).attr('data-id')] = $(this).attr('data-variant-title');
            bundle_items.push(bundle_item);
          })
          bundle_items.push({id: bundleWrapID, quantity: 1, properties: BundleWrapproperties});
          jQuery.post(window.Shopify.routes.root + 'cart/add.js', {
            items: bundle_items
          })
          .done(function() {
            $('.popup-bundle').removeClass('active');
            setTimeout(function() {
              $('.popup-bundle').hide();
            }, 350);
        ...
        $(this.selectors.addBundleToCart).off()
        .on('click', function(evt) {
          ...
          var bundle_items = [];
          var BundleWrapproperties = {bundle_id: __bundleID}
          var _$form = $(this).closest('form');
          _$form.find('.bundle-content__bundle_imagelist .bundle-content__bundle_imageitem.active').each(function() {
            var bundle_item = {};
            bundle_item['id'] = $(this).attr('data-variant-id');
            bundle_item['quantity'] = $(this).attr('data-qty');
            bundle_item['properties'] = {bundle_id: __bundleID, type: 'bundle'};
            BundleWrapproperties[$(this).attr('data-variant-id')] = $(this).attr('data-variant-title');
            bundle_items.push(bundle_item);
          })
          if (bundle_items.length != 0) {
            ...
            .done(function() {
              document.dispatchEvent(new CustomEvent('cart:build'));
              $('.bundle__item .icon-close').trigger('click')
              $('.js-drawer-open-cart').trigger('click');
              
              // $('body').trigger('added.ajaxProduct', $(this.selectors.addToCart));
              // document.dispatchEvent(new CustomEvent('ajaxProduct:added', {
              //   detail: {
              //     product: product
              //   }
      ...
      updateCartButton: function(evt) {
        ...
        if (variant) {
          if (variant.available) {
            ...
            $(this.selectors.addToBundle).removeClass(classes.disabled).prop('disabled', false);
            var defaultText = $(this.selectors.addToCartText).data('default-text');
            var defaultBundleText = $(this.selectors.addToBundleText).data('default-text');
            $(this.selectors.addToCartText).html(defaultText);
            $(this.selectors.addToBundleText).html(defaultBundleText);
          } else {
            ...
            $(this.selectors.addToBundle).addClass(classes.disabled).prop('disabled', true);
            console.log("addToBundleText", theme.strings.bundleSoldlOut)
            $(this.selectors.addToBundleText).html('SOLD OUT');
          }
        } else {
          ...
          $(this.selectors.addToBundle).addClass(classes.disabled).prop('disabled', true);
          $(this.selectors.addToBundleText).html('Unavailable');
        }
      },
  ...
  function  changeAjaxQuantity(bundle_id, qty, addition) {
    
    var classbundle = ".bundle_id-" + bundle_id
    var data = []
    $(classbundle).each(function() {
      ...
      if(addition) {
        ...
        var par = 'quantity=' + quantity + '&id=' + key
        data.push(par)
  ...
  function popupPriceUpdate($collection, __count, __price) {
  ...
  function findTabCount(event, selected_product) {
    var q = 0
    $(event).each(function(){
      if ($(this).attr('product-select') == selected_product) {
        q = q + 1
      }
    })
    return q
  ...
  $(document).on('click', '.bundle-content__bundle_text', function() {
    $(this).closest('#CollectionSection').find('form').each(function(idx) {
    var item_count = 0, total_price = 0;
    $(this).find(".bundle-content__bundle_imageitem.active").each(function() {
      item_count ++;
  ...
  $(document).ready(function() {
    ...
    sections.register('collection-bundle-section', theme.Product); 
  ...
  $(".ajax-cart-quantity").change(function() {
